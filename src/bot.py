...
class aclient(discord.Client):
    def __init__(self) -> None:
        ...
        self.activity = discord.Activity(
            type=discord.ActivityType.watching, name="/ai | /help"
        )
...
def run_discord_bot():
    ...
    @client.tree.command(name="ai", description="Have a chat with ChatGPT")
    ...
    async def private(interaction: discord.Interaction):
        ...
        if not isPrivate:
            ...
            await interaction.followup.send(
                "> **The next response will be sent here but unable to be seen by others. If you want to switch back to public mode, use `/public`**"
            )
        else:
            logger.info("You're already on private mode!")
            await interaction.followup.send(
                "> **Warn: You're already on private mode. If you want to switch to public mode, use `/public`**"
            )
    ...
    async def help(interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=False)
        await interaction.followup.send(
            ":star:** BASIC COMMANDS** \n    `/ai` Chat with ChatGPT!\n    `/public` switch ChatGPT to public mode \n   `/private` switch ChatGPT to private mode"
        )
